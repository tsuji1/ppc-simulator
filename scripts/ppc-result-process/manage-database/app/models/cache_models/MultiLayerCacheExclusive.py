...
class MultiLayerCacheParameter:
    ...
    def __init__(self , data:Dict|None=None):
        if data is not None:
            ...
            self.CachePolicies:List[str] = data["CachePolicies"]
        else:
            self.Type = ""
    ...
    def generate_hash(self) -> str:
        """
        CachePoliciesの要素、Type、CacheLayersのすべての要素を考慮してハッシュ値を生成する
        """
        # ハッシュ化するための文字列を作成
        hash_input = f"{self.Type}{self.CachePolicies}{[(layer.Type, layer.Size, layer.Refbits) for layer in self.CacheLayers.CacheLayers]}"
        
        # hashlibを使ってSHA256でハッシュ化
