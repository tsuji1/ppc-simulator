...
func (cache *FullAssociativeDstipNbitLRUCache) IsCachedWithFiveTuple(f *FiveTuple, update bool) (bool, *int) {
	...
	if hit {
		...
		if cache.debugMode {
			...
			hitIP, item := cache.routingTable.SearchLongestIP(dstIpAddress, 32, f.HitIPList, f.HitItemList)
...
func (cache *FullAssociativeDstipNbitLRUCache) CacheFiveTuple(f *FiveTuple) []*FiveTuple {
	...
	if cache.debugMode {
		...
		_, item := cache.routingTable.SearchLongestIP(ipaddress.NewIPaddress(maskDstIP), int(cache.Refbits), f.HitIPList,f.HitItemList)
